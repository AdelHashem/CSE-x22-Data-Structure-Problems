...
void bubbleSort(NodeM *start)
{
    ...
    do
    {
        ...
        while (ptr1->next != lptr)
        {
            ...
            if (ptr1->count == ptr1->next->count && ptr1->val->data !=' ')
            {
                if(ptr1->val->data < 97 && ptr1->next->val->data < 97)
                {
                    if(ptr1->val->data < ptr1->next->val->data)
                    {
                        swap(ptr1, ptr1->next);
                        swapped = 1;
                    }
                }
                else if(ptr1->val->data < 97)
                {
                    if(ptr1->val->data + 32 < ptr1->next->val->data)
                    {
                        swap(ptr1, ptr1->next);
                        swapped = 1;
                    }
                }else if(ptr1->next->val->data < 97)
                {
                     if(ptr1->val->data < ptr1->next->val->data +32)
                    {
                        swap(ptr1, ptr1->next);
                        swapped = 1;
                    }
                }else if(ptr1->val->data < ptr1->next->val->data)
                {
                    swap(ptr1, ptr1->next);
                swapped = 1;
                }
...
void BuildTree(Map *m)
{
    while(m->size > 1)
    {
        ...
        if(m->front->count == m->front->next->count && m->front->next->val->data == '$'&& m->front->val->data != '$')
        {
